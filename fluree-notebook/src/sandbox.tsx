      ...
      setTimeout(async () => {
        for (var i = 0; i < seedTransactions.length; i++) {
          ...
          if (seedTransactions[i]) {
            // staging
            newStaged = await stage(ledger, seedTransactions[i]);
          }
        ...
        <span className="xs:px-3 xs:scale-100 scale-[0.90]">
          <button
            ...
            className={`inline-flex items-center min-w-24 h-10 px-2 py-1 xs:px-5 xs:py-2
              ...
              justify-center text-sm xs:text-md font-medium focus:outline-none text-snowflake hover:bg-avalanche cursor-pointer disabled:bg-gray-400 bg-mid-depth 
              shadow-gray-700 disabled:bg-gray-600 ${
